...
public class UserAddIFrame extends JInternalFrame {
	...
	public UserAddIFrame() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (arg0.getSource() == button) {
					...
					int i = Dao.Insertoperator(username, sex, age, yajin,
							java.sql.Date.valueOf(date), tel, password);// Date.valueOf(date)把输入转化为date型
					if (i == 1) {
						JOptionPane.showMessageDialog(null, "添加成功！");// 如果影响了一行(插入成功)，显示插入成功对话框。
						doDefaultCloseAction();// 点击上句信息框体确定，则信息框体和添加用户框体都关闭。
					}
				}
			}
		...
		button_1.setText("取消");
		panel.add(button_1);
		button_1.addActionListener(new CloseActionListener());// 下面有它的监听事件,如果点击取消，则关闭信息框体和添加用户窗体

		

		final JPanel panel_1 = new JPanel();
		panel_1.setPreferredSize(new Dimension(0, 250));// 设置了一个高度为250的panel，宽度随窗口变化
		getContentPane().add(panel_1, BorderLayout.NORTH);// getContentPane()是获得窗口的面板,add()就是在面板上面添加控件

		final JPanel panel_2 = new JPanel();
		final GridLayout gridLayout = new GridLayout(0, 2);// 一行两列的布局方式
		gridLayout.setVgap(10);// 将组件之间的垂直间距设置为指定值10
		panel_2.setLayout(gridLayout);
		panel_2.setPreferredSize(new Dimension(280, 200));
		panel_1.add(panel_2);// 将pannel_2加入到pannel_1中.
		...
		panel_2.add(textField);// 将文本域加入到pannel_2中
		...
		panel_2.add(label_8);// 将label_8性别这个Jpanne加入到pannel_2中

		final JLabel label_2 = new JLabel();
		label_2.setText("年    龄：");
		panel_2.add(label_2);// 将年龄这个pannel加入到pannel_2

		textField_2 = new JTextField();
		textField_2.setDocument(new MyDocument(2));//运用自定义的MyDocument(max)方法，限制其大小
		...
		textField_3 = new JFormattedTextField(myfmt.getDateInstance());//JFormattedTextField只允许固定模式的输入格式
		textField_3.setValue(new java.util.Date());//设置默认值为当前时间
		...
		textField_4 = new JTextField(11);
		textField_4.setDocument(new MyDocument(11));//设置最大位数为11位
		textField_4.setToolTipText("请输入11位数字");//鼠标停留有tips
		...
		textField_5.setDocument(new MyDocument(18));
		textField_5.addKeyListener(new NumberListener());
		panel_2.add(textField_5);
		textField_5.setColumns(18);//JTextField 的 preferredSize 就是根据它的 column 数，和字体大小决定的
